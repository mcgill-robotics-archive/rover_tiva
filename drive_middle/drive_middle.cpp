...
int main(void) {
  ...
  HWREG(GPIO_PORTD_BASE + GPIO_O_LOCK) =
      GPIO_LOCK_KEY;  // In Tiva include this is the same as "_DD" in older
                      // versions (0x4C4F434B)
  ...
  /*    Configure the QEI to capture on both A and B, not to reset when there is
     an index pulse, configure it as a quadrature encoder,
        and doesn't swap signals PHA0 and PHB0 and set the maxmimum position as
     1999. (MAX_POS_CAPTURE_A_B = (ppr*4)-1.'*/
  // QEIConfigure(QEI0_BASE, (QEI_CONFIG_CAPTURE_A_B | QEI_CONFIG_NO_RESET |
  // QEI_CONFIG_QUADRATURE | QEI_CONFIG_NO_SWAP), 1999);
  ...
  ros::Publisher encoder_left("encoder_left", &drive_encoder_left_msg_stamped);
  ...
  while (1) {
    ...
    drive_encoder_left_msg_stamped.drive_encoder.taco_velocity =
        Lui32QEIVelocity;
    drive_encoder_right_msg_stamped.drive_encoder.taco_velocity =
        Rui32QEIVelocity;
