...
void bdc_init(BDC bdc) {
  ...
  PWMGenConfigure(bdc.PWM_BASE_IN1, bdc.PWM_GEN_IN1,
  ...
  ADCSequenceStepConfigure(bdc.ADC_BASE_CS, 3, 0,
                           bdc.ADC_CTL_CH_CS | ADC_CTL_IE | ADC_CTL_END);
...
void bdc_set_velocity(BDC bdc, int32_t velocity) {
  ...
  if (velocity == 0) {
    ...
    // No duty cycle scaling is needed, since input is directly the duty cycle
    PWMPulseWidthSet(bdc.PWM_BASE_IN1, bdc.PWM_OUT_IN1, 4001 - abs(velocity));
  }
}
